...
const Navbar: React.FC<HeaderProps> = () => {
  ...
  const isSubmitItemsActive = ["/reportlostitem", "/reportfounditem"].includes(
    location.pathname
  );

  // Check if current path is in Browse Items dropdown
  const isBrowseItemsActive = ["/lostitem", "/founditem"].includes(
    location.pathname
  );
  ...
  // Close dropdowns when navigating to different pages (not within same dropdown group)
  useEffect(() => {
    // Close Submit Items dropdown if navigating away from its pages
    if (!isSubmitItemsActive) {
      setOpen(false);
    }
    // Close Browse Items dropdown if navigating away from its pages
    if (!isBrowseItemsActive) {
      setopenBrowseritem(false);
    }
  ...
  // Function to handle dropdown item clicks in desktop
  const handleDropdownItemClick = (dropdownType: "submit" | "browse") => {
    if (dropdownType === "submit") {
      setOpen(false);
    } else {
      setopenBrowseritem(false);
    }
  ...
  return (
    <header className="w-full fixed z-50">
              ...
              <nav className="flex items-center space-x-6 xl:space-x-8">
                ...
                <div className="relative">
                  ...
                  {open && (
                    <div className="absolute top-full mt-2 w-44 bg-white border border-gray-200 rounded-lg shadow-xl z-50 overflow-hidden">
                      <Link
                        onClick={() => handleDropdownItemClick("submit")}
                        to="reportlostitem"
                ...
                <div className="relative">
                  ...
                  {openBrowseritem && (
                    <div className="absolute top-full mt-2 w-44 bg-white border border-gray-200 rounded-lg shadow-xl z-50 overflow-hidden">
                      <Link
                        onClick={() => handleDropdownItemClick("browse")}
                        to="lostitem"
                        className={`block px-4 py-3 text-sm hover:bg-gray-50 transition-colors ${
      ...
      {isMobileMenuOpen && (
        <div
          className={`lg:hidden h-screen bg-white border-t border-gray-200 shadow-lg transition-transform duration-300 ${
            isScrolled
        ...
        >
          <div className="px-4 sm:px-6 py-4 space-y-4">
            ...
            {/* Mobile Contact Link */}
             <Link to={'/contact'}         className={`block font-medium text-base py-3 px-4 rounded-lg transition-all text-left w-full ${
                isContactActive
                  ? "text-blue-600 bg-blue-50"
                  : "text-slate-700 hover:text-blue-600 hover:bg-gray-50"
              }`}
            ...
            {/* Mobile Login Button */}
             <Link to={"/LandingAuth"}
                 className=" flex items-center justify-center  w-full bg-slate-500 text-white py-2 px-8 font-medium text-base rounded-lg hover:bg-slate-600 transition-all duration-200 hover:shadow-md"
              >
            ...
            {/* Mobile Social Icons */}
            <div className="flex items-center justify-center space-x-6  mt-6 pt-4 pb-2 border-t border-gray-200">
              <Link
                to=""
